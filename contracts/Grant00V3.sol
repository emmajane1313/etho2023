...
import "@uma/core/contracts/optimistic-oracle-v3/interfaces/EscalationManagerInterface.sol";
...
import "@uma/core/contracts/data-verification-mechanism/interfaces/OracleAncillaryInterface.sol";
...
contract OptimisticOracleV3 is
    ...
    MultiCaller
{
    ...
    function assertTruth(
        ...
        require(
            _validateAndCacheIdentifier(identifier),
            "Unsupported identifier"
        );
        require(
            _validateAndCacheCurrency(address(currency)),
            "Unsupported currency"
        );
        require(
            bond >= getMinimumBond(address(currency)),
            "Bond amount too low"
        );
...

        {
            EscalationManagerInterface.AssertionPolicy
                memory assertionPolicy = _getAssertionPolicy(assertionId);
            require(!assertionPolicy.blockAssertion, "Assertion not allowed"); // Check if the assertion is permitted.
            EscalationManagerSettings storage emSettings = assertions[
                assertionId
            ].escalationManagerSettings;
            (
                emSettings.arbitrateViaEscalationManager,
                emSettings.discardOracle,
    ...
    function disputeAssertion(
        ...
        assertion.currency.safeTransferFrom(
            msg.sender,
            address(this),
            assertion.bond
        );
    ...
    function settleAssertion(bytes32 assertionId) public nonReentrant {
        ...
        if (assertion.disputer == address(0)) {
            ...
            int256 resolvedPrice = _oracleGetPrice(
                assertionId,
                assertion.identifier,
                assertion.assertionTime
            );
            ...
            address bondRecipient = resolvedPrice == numericalTrue
                ? assertion.asserter
                : assertion.disputer;
    ...
    function syncUmaParams(bytes32 identifier, address currency) public {
        ...
        cachedIdentifiers[identifier] = _getIdentifierWhitelist()
            .isIdentifierSupported(identifier);
        cachedCurrencies[currency].isWhitelisted = _getCollateralWhitelist()
            .isOnWhitelist(currency);
        cachedCurrencies[currency].finalFee = _getStore()
            .computeFinalFee(currency)
            .rawValue;
    }
    ...
    function getAssertionResult(
        ...
        if (
            ...
            assertion.escalationManagerSettings.discardOracle
        ) return false;
    ...
    function _getOracle(
        bytes32 assertionId
    ) internal view returns (OracleAncillaryInterface) {
        if (
    ...
    function _getEscalationManager(
        bytes32 assertionId
    ...
    function _isDisputeAllowed(
        bytes32 assertionId
    ) internal view returns (bool) {
        if (
            !assertions[assertionId].escalationManagerSettings.validateDisputers
        ) return true;
        address em = assertions[assertionId]
            .escalationManagerSettings
            .escalationManager;
        if (em == address(0)) return true;
        return
            EscalationManagerInterface(em).isDisputeAllowed(
                assertionId,
                msg.sender
            );
    }
    ...
    function _validateAndCacheIdentifier(
        bytes32 identifier
    ...
    function _validateAndCacheCurrency(
        address currency
    ) internal returns (bool) {
    ...
    function _callbackOnAssertionResolve(
        ...
        if (em != address(0))
            EscalationManagerInterface(em).assertionResolvedCallback(
                assertionId,
                assertedTruthfully
            );
    }
    ...
    function _callbackOnAssertionDispute(bytes32 assertionId) internal {
        ...
        if (em != address(0))
            EscalationManagerInterface(em).assertionDisputedCallback(
                assertionId
            );
    }
}
